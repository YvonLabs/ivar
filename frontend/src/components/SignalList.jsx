...
function SignalCard({ signal, onReview, readOnly }) {
  const [open, setOpen]           = useState(false)
  const [actioning, setActioning] = useState(null)
  const [toast, setToast]         = useState(null)

  const handle = async (action) => {
    if (actioning || readOnly || !onReview) return
    setActioning(action)
    try {
      ...
      {open && (
        <div className="signal-detail">
          ...
          <div className="signal-actions">
            {!readOnly && ACTIONS.filter(a => a.action !== signal.status).map(a => (
              <button
                key={a.action}
                className={a.cls}
            ...
            {signal.url && (
              <a
                ...
                style={{ marginLeft: readOnly ? 0 : 'auto' }}
              >
                source ↗
              </a>
            )}
    ...
    <div className="card">
      ...
      <div className="signals-body">
        {loading ? (
          ...
          signals.map(s => (
            <SignalCard key={s.id} signal={s} onReview={onReview} readOnly={readOnly} />
          ))
        )}
      </div>
    </div>
