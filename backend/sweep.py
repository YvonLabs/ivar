...
FLASH_MAX_AGE_DAYS = 365  # FLASH older than this is downgraded to PRIORITY
...
def should_auto_dismiss(signal) -> tuple[bool, str]:
    ...
    """
    # Keep FLASH only if stack match, or if no stack is configured
    if signal.severity == "FLASH":
        if signal.stack_match:
            return False, ""
        return True, "Auto-dismissed: FLASH but no stack match"
    ...
    for connector in get_active_connectors(db):
        try:
            ...
            try:
                ...
                if fc:
                    fc.last_fetched = datetime.utcnow()
                    fc.last_error   = None
            ...
            for raw in raw_signals:
                ...
                if existing:
                    # Re-triage pending rule-based signals if org stack changed
                    if existing.status == "pending" and existing.triage_method == "rule_based":
                        ...
                        # Downgrade old FLASH to PRIORITY
                        if existing.severity == "FLASH" and existing.published_at:
                            age = datetime.utcnow() - existing.published_at.replace(tzinfo=None)
                            if age.days > FLASH_MAX_AGE_DAYS:
                                existing.severity = "PRIORITY" if existing.stack_match else "ROUTINE"
                ...
                if severity == "FLASH" and raw.published_at:
                    age = datetime.utcnow() - raw.published_at.replace(tzinfo=None)
                    if age.days > FLASH_MAX_AGE_DAYS:
                        severity = "PRIORITY" if result.stack_match else "ROUTINE"
